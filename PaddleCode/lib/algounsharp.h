...
public:
    ...
    void runcorr_3D(float newslice[][11], loaf* myLoaf, int halfLoaf, float bound, float(*pfSpatialCorr)(int, int), float(*pfTemporalCorr)(int), float correction, float spatial_alpha_squared, float temporal_alpha_squared);
    float compute_rms(int option);
    float compute_rmscorr(float sigma, int mode, float(*pfCorr)(int, int), int mrow, int mcol);
    float compute_rmscorr_3D(float bound, int spaceMode, int timeMode, int halfLoaf, float(*pfSpatialCorr)(int, int), float(*pfTemporalCorr)(int), float spatial_alpha_squared, float temporal_alpha_squared);
