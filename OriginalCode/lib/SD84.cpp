...
September 2009
////
Some modifications to have a digital output, that tells you what the state of the grid currently is.
 
 Florent Lachaussée May 2013
 */
...
int SD84::setangles(int first_servo, int num_of_servos, double angles[], bool high_duty)
{
	int result = 0; 
	short int width; 
 ...
 // when commented out, avoid the crash after 5000 iterations when using a USB splitter FL May 2013
 //to send digital output (step function)
    /*if(num_of_servos==62){
        outputbuffer[130] = 0xAA;
        outputbuffer[131] = 0xA0;
        outputbuffer[132] = 0x55;
        outputbuffer[133]= SET_MODE;
        outputbuffer[134]= 84;
        outputbuffer[135]= 1;
        if (high_duty == false){
        outputbuffer[136]= OUTPUT_MODE0;
        }
        else {outputbuffer[136]= OUTPUT_MODE1;}
    result += thePort.send(outputbuffer, 6 + 2*num_of_servos + 7);
    }
    else {*/
